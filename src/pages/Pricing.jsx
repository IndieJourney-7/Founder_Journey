...
export default function Pricing() {
    return (
        <div className="flex-1 flex flex-col relative overflow-hidden bg-[#0F1F3D]">
            ...
            <div className="flex-1 max-w-6xl mx-auto w-full px-6 py-16 relative z-10 flex flex-col items-center">
                {/* Header */}
                <div className="text-center max-w-3xl mb-16">
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="inline-block px-4 py-1 rounded-full bg-white/5 border border-white/10 text-brand-gold text-sm font-bold mb-6"
                    >
...
function PricingCard({ title, price, period, description, features, buttonText, buttonLink, isPro, isWaitlist, delay, mostPopular }) {
    ...
    return (
        ...
        >
            {mostPopular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full bg-brand-gold text-brand-blue text-xs font-bold uppercase tracking-wider shadow-lg">
                    Most Popular
                </div>
            )}

            <div className="mb-8">
                <h3 className={`text-2xl font-bold mb-2 ${isPro ? 'text-brand-gold' : 'text-white'}`}>{title}</h3>
                <div className="flex items-baseline gap-1 mb-4">
                    <span className="text-4xl font-extrabold text-white">{price}</span>
                    <span className="text-white/50 text-sm font-medium">{period}</span>
                </div>
            ...
            <div className="flex-1 space-y-4 mb-8">
                {features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-3">
                        <div className={`mt-1 p-0.5 rounded-full ${isPro ? 'bg-brand-gold/20 text-brand-gold' : 'bg-white/10 text-white/50'}`}>
                            <Check size={12} />
                        </div>
            ...
            {isWaitlist ? ( // Pro Card Logic
                <button
                    ...
                    className="w-full py-4 rounded-xl bg-gradient-to-r from-brand-gold to-yellow-500 text-brand-blue font-bold shadow-lg hover:shadow-brand-gold/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2"
                >
                    <Sparkles size={18} />
                    {buttonText}
                </button>
                ...
                <Link
                    to={buttonLink}
                    className="w-full py-4 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold border border-white/10 transition-all flex items-center justify-center"
                >
                    {buttonText}
                </Link>
            )}
...
function WaitlistSection() {
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-2xl text-center py-12 border-t border-white/10"
        >
            ...
            <h2 className="text-3xl font-bold text-white mb-4">Early Beta Access</h2>
            ...
            {status === 'success' ? (
                <motion.div
                    ...
                    className="bg-brand-teal/10 border border-brand-teal/30 p-6 rounded-2xl inline-flex flex-col items-center gap-2 text-brand-teal text-center"
                >
                    <div className="flex items-center gap-2">
                        <Check size={24} />
                        <p className="font-bold text-lg">You're on the list!</p>
                    </div>
                    <p className="opacity-80">You'll be notified when Pro launches.</p>
                </motion.div>
            ) : (
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md mx-auto">
                    <input
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:border-brand-gold focus:outline-none transition-colors"
                        required
                    />
                    <textarea
                        value={feedback}
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:border-brand-gold focus:outline-none transition-colors min-h-[80px] resize-none"
                    />
                    ...
                    <button
                        ...
                        className="w-full px-6 py-3 rounded-xl bg-white text-brand-blue font-bold hover:bg-gray-100 transition-colors flex items-center justify-center gap-2 whitespace-nowrap disabled:opacity-70"
                    >
                        {status === 'loading' ? <Loader size={18} className="animate-spin" /> : 'Join Pro Waitlist'}
