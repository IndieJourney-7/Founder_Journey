...
export default function StepCard({ step, note, onUpdateStatus, onViewNote, onEdit, onDelete }) {
    ...
    return (
        <motion.div
            layout
            className={`p-4 rounded-xl border ${statusColors[step.status]} backdrop-blur-sm transition-all hover:bg-white/10 group relative ${isResolved ? 'cursor-pointer' : ''}`}
        ...
        >
            ...
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button
                    ...
                    className="p-1.5 rounded-md hover:bg-white/10 text-white/50 hover:text-brand-gold transition-colors"
                >
                    <Pencil size={14} />
                </button>
                <button
            ...
            <div className="flex justify-between items-start mb-2 pr-12">
                ...
                <div className="flex items-center gap-2">
                    {hasNote && (
                        <FileText className="w-4 h-4 text-brand-gold" title="Has reflection note" />
                    )}
                    <StatusIcon className={`w-5 h-5 ${step.status === 'success' ? 'text-brand-teal' :
            ...
            {step.description && (
                <p className="text-sm text-white/70 mb-4 line-clamp-2">{step.description}</p>
            )}
            ...
            {!isResolved && (
                <div className="flex gap-2">
                    <button
                        ...
                        className="flex-1 py-1.5 rounded bg-brand-teal/20 hover:bg-brand-teal/30 text-brand-teal text-xs font-bold transition-colors"
                    >
                        Success
                    </button>
                    <button
                        ...
                        className="flex-1 py-1.5 rounded bg-red-500/20 hover:bg-red-500/30 text-red-500 text-xs font-bold transition-colors"
                    >
                        Fail
                    </button>
                </div>
